    ...
    pub fn load(repo: &Repository, config: &Config) -> Result<Self> {
        ...
        let branch_name = git::current_branch_name(repo)
            ...
            .or_else(|| {
                ...
                config.defaults.current_stack.as_ref().and_then(|stack_name| {
                    config.defaults.branch_username.as_ref().map(|username| {
                        git::format_stack_branch(username, stack_name)
                    })
                })
            })
