...
pub fn create_branch_off_of_main(repository: Repository, branch_name: String) {
    ...
    let new_branch = repository
        .branch(
            &*branch_name,
            &main_branch.get().peel_to_commit().unwrap(),
            false,
        )
    ...
    repository
        .checkout_tree(&new_branch.get().peel(Commit).unwrap(), None)
        .expect("Could not check out new branch");

    // Set the new branch as the current head
    repository
        .set_head(&new_branch.get().name().unwrap())
        .expect("Could not set the new branch as the current head");

    print!("🎉 ");
    printcoln!("Branch \u{e0a0} [green|b|u]{}[:] created!", branch_name);
}
