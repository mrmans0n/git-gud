...
pub fn run() -> Result<()> {
    ...
    if stack.len() < 2 {
        println!(
            "{}",
            style("Need at least 2 commits to reorder.").dim()
        );
        return Ok(());
    }
    ...
    todo_content.push_str("# The first commit will be at the bottom of the stack (closest to base).\n");
    ...
    for sha in &new_order {
        if !old_order.iter().any(|s| s.starts_with(*sha) || sha.starts_with(*s)) {
    ...
    for sha in &new_order {
        // Find the full SHA
        let full_sha = stack.entries.iter()
