...
- `--update-title`: Update PR/MR titles to match commit messages.
...
Supports global config at `~/.config/gg/config.json` for shared defaults across repos. New fields: `sync_draft` (create PRs as drafts), `sync_update_descriptions` (update PR descriptions on re-sync), and `sync_update_title` (update PR titles on re-sync, default false).
