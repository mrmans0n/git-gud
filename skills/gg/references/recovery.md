...
1. Resolve conflicts, stage only resolved files, then run `gg continue`.
   After continuing a conflicted `gg rebase`, inspect for the operation's
   `gg-rebase-autostash`; restore it or explicitly surface it before declaring
   the worktree clean.
2. Use `gg abort` only when aborting is requested or necessary to return safely.
   After aborting a conflicted `gg rebase`, inspect for the operation's
   `gg-rebase-autostash`; restore it or explicitly surface it before declaring
   the worktree clean.
...
After recovery, verify refs, `HEAD`, operation record, stack order, dirty state,
and whether any `gg-rebase-autostash` remains.
