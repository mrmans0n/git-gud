...
The positive fallback instruction establishes a known gg-positive context and measures what the current skill requires after that point; it does not disclose a router design, expected answer, or baseline hypothesis. It therefore does not test whether a positive prompt activates gg autonomously. Scenario 8 is the unbiased activation control: it received neither a skill path nor a gg mention and did not introduce gg. Collaboration returns evaluator final messages rather than tool-event streams; the [tracked baseline evaluator evidence](2026-07-28-gg-agent-skill-router-baseline-evidence.md) retains every complete final output and fresh context identifier. The original dispatch wrapper was not retained, and the evidence says so rather than reconstructing it. The file-consultation lines below are direct evaluator output, not inferred tool events.
...
| Behind-base sync | Fail (router and workflow) | Consulted only the 435-line `SKILL.md` and proposed nonexistent `gg rebase --json` before JSONL sync. |
...
Operationally, scenarios 1, 2, and 4 through 7 gave safe, structured, truthful
answers: read-only inspection remained non-mutating; local absorb was separated
from remote sync; immutable and remote-undo cases withheld override authority;
landing required confirmation; and GitLab polling was reported as incomplete.
Scenario 3 correctly sequenced rebase before publication but selected the
nonexistent `gg rebase --json` flag, so it is also a workflow failure. The
shared routing failure remains: every relevant gg scenario starts with the same
435-line entrypoint, and one scenario additionally required a 1,111-line
reference.
...
Complete output: [tracked baseline evidence, scenario 1](2026-07-28-gg-agent-skill-router-baseline-evidence.md#scenario-1--inspection-only).
...
Complete output: [tracked baseline evidence, scenario 2](2026-07-28-gg-agent-skill-router-baseline-evidence.md#scenario-2--multi-commit-edit).
...
Complete output: [tracked baseline evidence, scenario 3](2026-07-28-gg-agent-skill-router-baseline-evidence.md#scenario-3--behind-base-sync).
...
Complete output: [tracked baseline evidence, scenario 4](2026-07-28-gg-agent-skill-router-baseline-evidence.md#scenario-4--immutable-rewrite).
...
Complete output: [tracked baseline evidence, scenario 5](2026-07-28-gg-agent-skill-router-baseline-evidence.md#scenario-5--interrupted-remote-touching-undo).
...
Complete output: [tracked baseline evidence, scenario 6](2026-07-28-gg-agent-skill-router-baseline-evidence.md#scenario-6--ambiguous-landing-authority).
...
Complete output: [tracked baseline evidence, scenario 7](2026-07-28-gg-agent-skill-router-baseline-evidence.md#scenario-7--gitlab-merge-train).
...
Complete output: [tracked baseline evidence, scenario 8](2026-07-28-gg-agent-skill-router-baseline-evidence.md#scenario-8--negative-activation).
...
- Preserve rebase-before-sync, JSONL monitoring, immutability stops,
  confirmation gates, and non-terminal merge-train wording.
- Correct scenario 3's nonexistent `gg rebase --json`: `rebase` has no JSON
  flag, so the routed workflow must use ordinary `gg rebase` and reserve
  structured output for the surrounding inspections and sync.
